...
class MultiHeadCrossAttention(MultiHeadAttention):
    def __init__(self, channelY, channelS, downsample_factor=4):
    ...
    def forward(self, Y, S):
        ...
        Spe = self.Spe(S)
        S = S + Spe
        S1 = self.Sconv(S).reshape(Yb, Sc, Yh * Yw).permute(0, 2, 1)  # [Yb, Yh*Yw, Sc]
        V = self.value(S1).permute(0, 2, 1)  # [Yb, Sc, Yh*Yw]
        ...
        Q = self.query(Y1)
        K = self.key(Y1)
        A = self.softmax(torch.bmm(Q.permute(0, 2, 1), K) / math.sqrt(Sc))
...
class U_Transformer(nn.Module):
    def __init__(self, in_channels, classes, bilinear=True):
        ...
        self.MHSA = MultiHeadSelfAttention(1024)
        self.up1 = TransformerUp(1024, 512)
        self.up2 = TransformerUp(512, 256)
        self.up3 = TransformerUp(256, 128)
        self.up4 = TransformerUp(128, 64)
...
# model = U_Transformer(in_channels=3, classes=1)
